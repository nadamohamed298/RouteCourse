...
const cancelModalButton = document.querySelector("#cancel-btn") as HTMLButtonElement;
const submitTaskButton = document.querySelector("#submit-btn") as HTMLButtonElement;
const submitButtonText = document.querySelector("#submit-btn-text") as HTMLElement;
const modalTitle = document.querySelector("#modal-title") as HTMLElement;

const tasksTodoContainer = document.querySelector("#tasks-todo") as HTMLElement;
const tasksInProgressContainer = document.querySelector("#tasks-in-progress") as HTMLElement;
const tasksCompletedContainer = document.querySelector("#tasks-completed") as HTMLElement;
...
const taskDueDateInput = document.querySelector("#task-due-date") as HTMLInputElement;
const taskDescriptionTextarea = document.querySelector("#task-description") as HTMLTextAreaElement;
const characterCountDisplay = document.querySelector("#char-count") as HTMLElement;
const titleErrorMessage = document.querySelector("#title-error") as HTMLParagraphElement;
const dateErrorMessage = document.querySelector("#date-error") as HTMLParagraphElement;

// Types
type Priority = "Low" | "Medium" | "High";
type TaskStatus = "To Do" | "In Progress" | "Completed";
type DueDateStatus = "Due Soon" | "Overdue" | "Done" | "On Track";
...
let tasks: Task[] = [];
let currentlyEditingTaskId: number | null = null;
...
function calculateDueDateStatus(dueDate: Date, status: TaskStatus): DueDateStatus {
...
function getRelativeTime(date: Date): string {
  ...
  if (differenceInDays === 1) return "1d ago";
  if (differenceInDays < 7) return `${differenceInDays}d ago`;
  if (differenceInDays < 30) return `${Math.floor(differenceInDays / 7)}w ago`;
  return date.toLocaleDateString();
}

function formatFullCreatedDate(date: Date): string {
...
function isDueDateValid(dueDateString: string): boolean {
...
function showTitleError(message: string): void {
  if (!titleErrorMessage) return;
  titleErrorMessage.textContent = message;
  titleErrorMessage.classList.remove("hidden");
  if (taskTitleInput) setInputError(taskTitleInput, true);
}

function hideTitleError(): void {
  if (!titleErrorMessage) return;
  titleErrorMessage.textContent = "";
  titleErrorMessage.classList.add("hidden");
  if (taskTitleInput) setInputError(taskTitleInput, false);
}
...
function showDateError(message: string): void {
  ...
  if (taskDueDateInput) setInputError(taskDueDateInput, true);
}

function hideDateError(): void {
  if (!dateErrorMessage) return;
  dateErrorMessage.textContent = "";
  dateErrorMessage.classList.add("hidden");
  if (taskDueDateInput) setInputError(taskDueDateInput, false);
}
...
function clearTaskForm(): void {
  ...
  if (characterCountDisplay) characterCountDisplay.textContent = "0/500";
...
// Add Task Modal
function openTaskModal(taskToEdit?: Task): void {
  ...
  if (taskToEdit) {
    ...
    if (taskPrioritySelect) taskPrioritySelect.value = taskToEdit.priority.toLowerCase();
    if (taskDueDateInput && taskToEdit.dueDate) {
      taskDueDateInput.value = taskToEdit.dueDate.toISOString().split("T")[0];
      setInputError(taskDueDateInput, false);
    }
    if (taskDescriptionTextarea) taskDescriptionTextarea.value = taskToEdit.description;
    ...
    if (submitTaskButton) {
      const icon = submitTaskButton.querySelector("i");
      if (icon) icon.className = "fa-solid fa-save";
    }
  } else {
    clearTaskForm();
  }
...
function updateColumnCounters(todoCount: number, inProgressCount: number, completedCount: number): void {
  const columns = [
...
function renderAllTasks(): void {
  if (!tasksTodoContainer || !tasksInProgressContainer || !tasksCompletedContainer) return;
  ...
  const renderTasks = (container: HTMLElement, taskList: Task[]) => {
...
function validateForm(): boolean {
  if (!taskTitleInput || !taskDueDateInput) return false;
...
function addOrUpdateTask(): void {
  if (!taskTitleInput || !taskPrioritySelect || !taskDueDateInput || !taskDescriptionTextarea) return;
...
function changeTaskStatus(taskId: number, newStatus: TaskStatus): void {
...
function setupEventListeners(): void {
  ...
  document.addEventListener("click", (e) => {
    ...
    const statusButton = target.closest(".status-btn") as HTMLElement | null;
    if (statusButton) {
      const taskId = Number(statusButton.dataset.taskId);
      const newStatus = statusButton.dataset.status as TaskStatus;
      if (taskId && newStatus) {
